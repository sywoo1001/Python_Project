...
def classify(df):
    ...
    for v in count.values():
        if v>50 and v<100:
            danger.append('주의<br>======<br><고려 조치><br>'
                      '마스크 착용 의무화<br>'
                      '음식 섭취 금지 <br>'
                      '21시 이후 운영 중단<br>'
                      '시설 면적 4m^2 혹은 8m^2 당 한명으로 인원 제한<br>'
                      '기타 등등<br>'
                      '참조:http://ncov.mohw.go.kr/shBoardView.do?brdId=6&brdGubun=64&ncvContSeq=4224')
        elif v>=100:
            danger.append('위험<br>======<br><고려 조치><br>운영제한 및 집합금지'
                      '<br>참조:http://ncov.mohw.go.kr/shBoardView.do?brdId=6&brdGubun=64&ncvContSeq=4224')
        else:
            danger.append('안전<br>======<br>현행유지')
